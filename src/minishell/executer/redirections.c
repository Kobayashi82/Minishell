...
/*   Updated: 2026/01/15 16:18:27 by vzurera-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (in)
		safe_dup2(&data->bk_in, STDIN_FILENO, 0);
	if (out)
		safe_dup2(&data->bk_out, STDOUT_FILENO, 0);
	if (err)
		safe_dup2(&data->bk_err, STDERR_FILENO, 0);
}
...
static int	output(t_data *data, t_token *tk, t_redir *redir, bool forked)
{
	...
	if (fd == -1 && forked)
		...
		return (1);
	return (safe_dup2(&fd, STDOUT_FILENO, 1), 0);
}
...
int	set_redirections(t_token *tk, bool f)
{
	...
	safe_dup2(&tk->data->bk_in, STDIN_FILENO, 0);
	safe_dup2(&tk->data->bk_out, STDOUT_FILENO, 0);
	close_fds(tk, 1);
	safe_dup2(&tk->fd[0], STDIN_FILENO, 1);
	safe_dup2(&tk->fd[1], STDOUT_FILENO, 1);
	c = tk->redir;
	while (c)
	{
		if (!tk->data->main && !tk->data->is_subshell && c->type == DLT)
		{
			...
			safe_dup2(&tk->data->extra, STDIN_FILENO, 1);
			tk->data->extra = 0;
		}
